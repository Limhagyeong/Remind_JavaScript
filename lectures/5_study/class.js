...
// const ive = iveMembers.map((member) => {
//   return new FemaleIdol(member.name, member.year);
// });

// const bts = btsMembers.map((member) => {
...
// const btsGroup = new IdolGroup('방탄소년단', bts);
...
const allTogether = new Country('대한민국', [
  ...
  new IdolGroup(
    ...
    btsMembers.map((member) => {
      return new Idol(member.name, member.year);
    })
  ),
]);
